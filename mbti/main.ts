...
export function findMBTI(responses: string[]): string {
  ...
  questions.forEach((question, index) => {
    ...
    if (response === "매우 아니다") scores[question.disagree] += 2;
    else if (response === "아니다") scores[question.disagree] += 1;
    else if (response === "보통이다") return;
    else if (response === "그렇다") scores[question.agree] += 1;
    else if (response === "메우 그렇다") scores[question.agree] += 2;
  });
...
const mbtiResult = findMBTI(userResponses);
